...
# Setup Istio
export WORKDIR=`pwd`
ISTIOCTL_CMD=${WORKDIR}/${ASM_PACKAGE}/bin/istioctl

# Setup kubeconfig
echo -e "Setting up kubeconfig at ./tempkubeconfig"
...
if [[ ${CLUSTER} != ${TARGET_CLUSTER} ]]; then
    ...
    ${ISTIOCTL_CMD} x create-remote-secret --kubeconfig ${KUBECONFIG} --context=gke_${PROJECT_ID}_${LOCATION}_${CLUSTER} --name=${CLUSTER} > ${WORKDIR}/manifests/secret-kubeconfig-${CLUSTER}.yaml
    kubectl apply -f ${WORKDIR}/manifests/secret-kubeconfig-${CLUSTER}.yaml --kubeconfig=${KUBECONFIG} --context=gke_${PROJECT_ID}_${TARGET_LOCATION}_${TARGET_CLUSTER}
else
