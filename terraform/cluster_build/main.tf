...
locals {
  ...
  ce_service_account       = format("service-%s@container-engine-robot.iam.gserviceaccount.com", data.google_project.project.number)
...
module "kms" {
  ...
  owners = [
    "serviceAccount:${local.ce_service_account}",
  ]
}
