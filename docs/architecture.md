...
When using the cli defaults, `gkekitctl create`, a Private Cluster in a Standalone VPC with the settings below is built. A config file can be used to customize the install, the spec and examples are kept in this [folder](cli/samples). Definitions of each configuration choice in the spec are stored in the [configuration.md](docs/configuration.md) doc. The rest of this doc goes over all the non-optional security features every cluster is bootstraped with.
